...
function learningLinksFor(keyword) {
  ...
  return [
        ...
        impact: 'High \u2014 closes the loop from "what\u2019s missing" to "here\u2019s how to learn it," scores the actual generated r\u00e9sum\u00e9, and gives users a portable Markdown learning tracker \u2014 all with zero server-side storage.'    },
    {
        ...
        role: 'Front-End',
        effort: '120 min',
        summary: 'Replaced the single hard-coded résumé layout with a VISUAL gallery of six distinct templates rendered as live SVG thumbnails (font + accent colour + header & heading treatment), plus role-aware "Top pick" badges so users do not have to guess. Users click the design they want (like Word/Google Docs); the chosen template drives both the PDF and Word outputs, persists across sessions, and flows through single, bulk, publish, and Ollama-cloud rebuilds.',
...
function recommendResumeTemplates(jd, profile) {
  ...
  return [...new Set([...picks,'classic-ats'])].slice(0,3);
        ...
        impact: 'High — users now SEE each résumé design as a thumbnail and click the one they want (with role-based "Top pick" badges), the AI output is no longer a single plain layout, and the selected design carries through every output path including the published GitHub repo.'
    }
];
